...
df = pd.read_parquet("datos/processed/snies/Matriculados_total.parquet")
...
print("\n" + "="*60)
print("MATRICULADOS TOTALES")
print("="*60)

print(f"\nTotal matriculados: {df['matriculados'].sum():,}")
...
fig = px.line(
    ...
    markers=True,
    text="matriculados",
    title="Evolución temporal de la matrícula en educación superior en Colombia (2015–2024)"
)
...
plt.title(
    "Evolución de matriculados por año",
    fontsize=20,
    fontweight="bold"
)
...
fig = px.bar(
    ...
    x="genero",
    y="matriculados",
    title="Distribución de la matricula por género en educación superior en Colombia (2015–2024)",
    text="matriculados",
    color="genero"
)
...
plt.ylabel("Matriculados", fontsize=12)
...
sector_df = (
    df.groupby("sector")["matriculados"]
    .sum()
    .reset_index()
    .sort_values(by="matriculados", ascending=False)
)
...
fig = px.pie(
    ...
    names="sector",
    values="matriculados",
    title="Participación de la matrícula según sector en Colombia (2015–2024)",
    hole=0.4
)
...
area_df = (
    ...
    .groupby("area_conocimiento")["matriculados"]
    .sum()
    .reset_index()
    .sort_values(by="matriculados", ascending=False)
)
...
fig = px.bar(
    ...
    y="area_conocimiento",
    orientation="h",
    title="Distribución de matriculados por área de conocimiento en Colombia (2015–2024)",
    text="matriculados",
    color="matriculados",
)
...
# Diseño
plt.title(
    "Distribución de matriculados por área de conocimiento en Colombia (2015–2024)",
    fontsize=16,
    fontweight='bold'
)

plt.xlabel("Matriculados", fontsize=12)
...
# Top departamentos
depto_df = (
    df.groupby("depto_programa")["matriculados"]
    .sum()
    .reset_index()
    .sort_values(by="matriculados", ascending=False)
    .head(15)
...
fig = px.funnel(
    ...
    x="matriculados",
    y="depto_programa",
    title="Distribución territorial de la matrícula por departamento en Colombia (2015–2024)"
)
...
# Diseño
plt.title(
    "Top departamentos por matriculados",
    fontsize=16,
    fontweight='bold'
)
...
plt.savefig("artifact/visualizaciones/matricula_departamento.png", dpi=300, bbox_inches="tight")
plt.show()
...
# Top 15 programas
programas_df = (
    df.groupby("nombre_programa")["matriculados"]
    .sum()
    .reset_index()
    .sort_values(by="matriculados", ascending=False)
    .head(15)
...
fig = px.funnel(
    ...
    x="matriculados",
    y="nombre_programa",
    title="Programas con mayor número de matriculados en Colombia (2015–2024)"
)
...
plt.hlines(
    ...
    xmin=0,
    xmax=programas_plot["matriculados"],
    linewidth=3,
    alpha=0.7
)
...
for i, row in programas_plot.iterrows():
    ...
    plt.text(
        row["matriculados"] + 20000,
        row["nombre_programa"],
        f'{int(row["matriculados"]):,}',
        va='center',
        fontsize=10,
        fontweight='bold'
    )
...
inst_df = (
    df.groupby("nombre_institucion")["matriculados"]
    .sum()
    .reset_index()
    .sort_values(by="matriculados", ascending=False)
    .head(15)
...
fig = px.sunburst(
    ...
    values="matriculados",
    title="Instituciones con mayor participación en la matrícula en Colombia (2015–2024)"
)
...
for i in range(len(inst_plot)):
    ...
    plt.hlines(
        ...
        xmin=0,
        xmax=inst_plot.iloc[i]["matriculados"],
        alpha=0.3,
        linewidth=2
    )
...
# Diseño
plt.title(
    "Matriculados por institución en Colombia (2015–2024)",
    fontsize=18,
    fontweight='bold'
)
...
plt.title(
    "Evolución temporal de la matrícula por área de conocimiento en Colombia (2015–2024)",
    fontsize=20,
    fontweight='bold',
    pad=20
)
...
mas["matriculados"] = -mas["matriculados"]
...
max_value = max(
    fem["matriculados"].max(),
    abs(mas["matriculados"]).max()
)
...
for _, row in mas.iterrows():
    ...
    ax.text(
        row["matriculados"] - (offset * 7),
        row["area_conocimiento"],
        f'{abs(int(row["matriculados"])):,}',
        va='center',
        fontsize=10,
        fontweight='bold'
    )
...
plt.savefig("artifact/visualizaciones/Brechas.png", dpi=300, bbox_inches="tight")
plt.show()